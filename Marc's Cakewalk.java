...
class Result {
    ...
    // Write your code here
        ...
        {
            for (int j = i + 1; j < calorie.size(); j++) 
            {
                if (calorie.get(j) > calorie.get(i)) 
                {
                    int temp = calorie.get(j);
                    calorie.set(j, calorie.get(i));
                    calorie.set(i, temp);
                }
            }
        }
    
    // Calculate the minimum number of miles required
        long miles = 0;
